...
namespace Warms_Game
{
    internal class Player
    {
        ...
        public void SetAlive(bool isAlive)                      //죽음체크
        {
            int[,] P_1 = new int[player_xpos, player_ypos];
            int[,] P_2 = new int[player_xpos2, player_ypos2];

            /*if (player_xpos == GameLoop.BOARD_WIDTH-4 || player_ypos == GameLoop.BOARD_HEIGHT-3 || player_xpos == 4 || player_ypos == 3)
                isAlive = false;
            if (player_xpos2 == GameLoop.BOARD_WIDTH-4 ||  player_ypos2 == GameLoop.BOARD_HEIGHT-3 | player_xpos2 == 4 || player_ypos2 == 3)
                isAlive = false;*/
            if ((player_xpos == player_xpos2)&& (player_ypos == player_ypos2)) //플레이어끼리 충돌 체크 안됨
                this.isAlive = false;
            else
                this.isAlive = true;
        }
        ...
        public void Awake()
        {
            ...
            Tail_x = new int[100];
            Tail_y = new int[100];
            Tail_x_2 = new int[100];
            Tail_y_2 = new int[100];
        ...
        public void Update()
        {
            ...
            switch (dir)
            {
                case 0: //0. 오른 1. 위 2. 왼 3. 아래
                    ...
                    player_xpos += 2;
                    if (player_xpos > GameLoop.BOARD_WIDTH - 2)
                    {
                        player_xpos = GameLoop.BOARD_WIDTH - 2;
                        isAlive = false; // 벽에 닿으면 죽음 판정 -> 게임오버
                    }
                    if (player_xpos2 == player_xpos && player_ypos2 == player_ypos)
                        isAlive = false; // 적과 충돌 -> 게임오버
                    break;
                ...
                case 3:
                    player_ypos++;
                    if (player_ypos > GameLoop.BOARD_HEIGHT - 2)
                    {
                        player_ypos = GameLoop.BOARD_HEIGHT - 2;
                        isAlive = false;
                    }
            ...
            switch (dir2)
            {
                ...
                case 0:
                    player_xpos2 += 2;
                    if (player_xpos2 > GameLoop.BOARD_WIDTH - 2)
                    {
                        player_xpos2 = GameLoop.BOARD_WIDTH - 2;
                        isAlive = false; // 벽에 닿으면 죽음 판정 -> 게임오버
                    }
                    if (player_xpos2 == player_xpos && player_ypos2 == player_ypos)
                        isAlive = false; // 적과 충돌 -> 게임오버
                ...
                case 3:
                    player_ypos2++;
                    if (player_ypos2 > GameLoop.BOARD_HEIGHT - 2)
                    {
                        player_ypos2 = GameLoop.BOARD_HEIGHT - 2;
                        isAlive = false;
                    }
