...
namespace Warms_Game
{
    internal class GameLoop
    {
        ...
        public void Update()
        {
            ...
            if (curTime - oldTime > 100)        //조건문 사용해서 1/10초마다 한번씩 실행
            {
                player.Update();
        ...
        public void Render()
        {
            ...
            /*//범위 체크해서 반짝임 사라지게 하기
            for (int i = 1; i < BOARD_HEIGHT; i++)//j는 x좌표 i는 y좌표
            {
                for (int j = 1; j < BOARD_WIDTH; j++)
                {
                    if ((j == player.GetXpos()) && (i == player.GetYpos()))//player1감시
                    {
                    ...
                    else if (player.Tail_x[j] && player.Tail_y[i])//player1꼬리 감시
                    {
                        Console.SetCursorPosition(j, i);
                    ...
                    else if ((j == player2.GetXpos2()) && (i == player2.GetYpos2()))//player2감시
                    {
                        Console.SetCursorPosition(j, i);
                    ...
                    else if (player.Tail_x[j] && player.Tail_y[i])//player2꼬리 감시
                    {
                        Console.SetCursorPosition(j, i);
